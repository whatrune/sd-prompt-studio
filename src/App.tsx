            ...
            <div className="selected-outline">
              {selectedSections.map(section=>{
                ...
                return <section className={`selected-layer context-${section.kind.toLowerCase()} interactive ${expanded?'expanded':'collapsed'}`} key={section.id}>
                  ...
                  {store.blocks.length>1&&section.targetId!=='scene'&&<div className="context-character-metadata"><label className="context-position-inline" onClick={event=>event.stopPropagation()}><span>配置</span><select aria-label="配置" value={store.blocks.find(block=>block.id===section.targetId)?.position??'center'} onClick={event=>event.stopPropagation()} onChange={event=>store.setSubjectPosition(section.targetId,event.target.value as 'left'|'center'|'right')}><option value="left">左側</option><option value="center">中央</option><option value="right">右側</option></select></label></div>}
