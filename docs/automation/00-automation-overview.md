...
- The consumption claim makes the authority single-use before the protected mutation. After a second complete rebind of authority, Review, main, PR, paths, checks, and threads, the route passes a module-authenticated action to the existing `executeReadyTransitionOperatorV1`. That operator remains the only owner of `markPullRequestReadyForReview`, the before/after Draft-state guards, and exact-HEAD refetch. The workflow still relies on GitHub's real `pull_request.ready_for_review` event; no event is synthesized.
