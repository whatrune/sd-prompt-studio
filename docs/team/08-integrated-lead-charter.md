...
Integrated Lead owns continuation after dispatching an owner or reviewer. Existing wait targets/cursors are optional wake-up diagnostics, not authority. When supplied, a repeated consumed cursor produces no additional action. Every advancement fresh-binds the exact Task, PR, HEAD, branch, worktree, and scope. Validated implementation proceeds to commit/publish; current checks PASS to Fresh Review; findings to the same Worker; replacement Fresh approval to exact corrected-thread resolution and direct Review publication/preflight; successful preflight to MERGE_READY and the Product Owner STOP.
