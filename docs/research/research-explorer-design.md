...
flowchart LR
    ...
    E --> F["Local Companion Service API"]
...
正式サポート方式は**Local Companion Service**とする。

```text
Research Explorer Frontend
└─ Local Companion Service API
   ├─ Artifact Discovery
...
- Frontend: Navigation、Artifact表示、Read Model rendering
- Local Companion Service: Artifact Discovery、Index生成、Path/Contract Validation、Artifact Read、Hash Verification
- Research Repository Artifact: 正本。Frontend bundleやDerived Indexへ本文を複製しない

FrontendはLocal Companion Serviceが到達不能な場合、実Dataが空であるかのように扱わず、`service unavailable`を明示する。FrontendからRepository filesystemへ直接fallbackする方式はサポートしない。API transport、起動方式、port、認証、Index Formatの詳細はPR74で決定する。
...
flowchart TD
    ...
    UI["Research Explorer Frontend"] --> SVC
```
...
- Local Companion Serviceの起動方式、API transport、port、認証方式
