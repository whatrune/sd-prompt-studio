...
- A terminal implementation Result Handoff with complete exact-HEAD validation MUST immediately dispatch prepublication Independent Review. That review is a task-local unchanged-publication gate, not the authoritative Merge Review. Its approval MUST bind a clean exact commit; publication MUST push that exact commit without amend, rebase, additional commit, or tree change and MUST create one non-Draft PR under explicit publication authority.
