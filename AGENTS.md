...
- Normal Task execution follows the [Shared Role Execution Contract](docs/team/13-shared-role-execution-contract.md#normal-task-lifecycle). Task start and Merge are the two normal Product Owner decisions. Integrated Lead remains the continuation owner; existing wait signals are optional diagnostics, not authority. Exact live identity, validation, current checks and Fresh Review remain mandatory.
- Checks PASS dispatches Fresh Review; findings return to the same owning Worker. A validated same-Task correction commits only its authorized delta and uses one leased successor push to the existing PR. Replacement Fresh Review precedes exact corrected-thread resolution and canonical Review publication/preflight.
...
- After confirmed Merge and successful post-Merge verification, Integrated Lead MUST invoke `scripts/remove-task-worktree-after-merge-v1.ps1` through the exact host-owned bundled `pwsh` executable. Before cleanup, it MUST probe that opaque executable and require PowerShell Core in the bounded `7.6.x` line with `Major = 7`, `Minor = 6`, and `Patch >= 4`, plus callable `[IO.Path]::GetRelativePath`; Windows PowerShell 5.1, a PATH-selected `pwsh`, alternate-shell fallback, and retry through another shell are prohibited. The existing helper remains the sole cleanup-semantics owner.
- After post-Merge verification, local-main synchronization, and terminal Task-worktree cleanup all PASS, Integrated Lead MUST evaluate the exact Canonical Task's predelegated closure policy. `AUTO_CLOSE_COMPLETED` closes only that Task Issue with GitHub reason `completed`, preserves its body byte-for-byte, and directly refetches the closed resource; an already-closed Issue is zero-mutation PASS. `KEEP_OPEN`, and every legacy Task lacking the policy field, is zero-mutation PASS. Ordinary newly serialized Tasks default to `AUTO_CLOSE_COMPLETED` unless their Task authority explicitly selects `KEEP_OPEN`. This closure operation is predelegated by the Canonical Task and is not inferred from Merge Decision authority. It uses the single canonical Issue-close transport, performs no retry after an ambiguous mutation, and has no fallback route. Any closure failure is reported separately and MUST NOT invalidate the confirmed Merge, successful local-main synchronization, or successful terminal cleanup.
