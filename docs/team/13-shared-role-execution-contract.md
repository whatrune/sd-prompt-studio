...
normative dependencyは必ず`consumer -> owner`とする。ownership declaration、navigation backlink、non-normative exampleはdependency edgeとして数えない。literal Markdown link graphとnormative dependency graphは別に検査する。

```text
templates / routing / role charters / review overlay -> 13 shared execution
13 shared execution -> 11 delegation/result shape and status
role charters -> 00 role taxonomy and decision ownership
05 git lifecycle -> 13 same-task authority
automation documents -> human contracts
```
