...
const stateProblem=(v:unknown,profiles?:GenericProgressRunnerProfilesV1)=>{
  ...
  if(profiles){
    p=profileProblem(profiles);if(p)return `/profiles${p}`
    if(!s.allowed_transition_classes.every(t=>profiles.route_binding_table.bindings.some(r=>r.transition_class===t)))return '/allowed_transition_classes:invalid_cross_input_binding'
    if(s.active_role_binding!==null&&profiles.route_binding_table.bindings.filter(r=>canonicalizeContinuousOrchestrationJsonV1(r)===canonicalizeContinuousOrchestrationJsonV1(s.active_role_binding)).length!==1)return '/active_role_binding:invalid_cross_input_binding'
  }
  return undefined
}
...
function reduceCoreContinuousOrchestrationV1(
  ...
  const s=sa.value,e=ea.value;evaluation=va.value
  ...
  const replay=s.replay_ledger.entries.find(x=>x.semantic_event_digest===e.semantic_event_digest)
  ...
  if(e.predecessor_event_id_or_null!==s.event_cursor.last_event_id_or_null){
    const decision=stop('canonical_conflict','canonical_conflict',pa.value.authority_projection_profile.collector_role_id)
    return frozen({admission:null,decision,state:s,cas_projection:null,terminal_no_mutation:true})
  }
