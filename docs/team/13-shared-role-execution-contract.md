...
A Product Owner may predelegate exactly one later `REVIEW_AUTHORITY_PUBLICATION` assignment materialization through the existing `task_assignment` contract. The predelegation is materialization-only and inactive until one new consumed terminal cursor carries a Fresh exact-HEAD `APPROVE / 0 / 0 / 0`. It binds the stable Task, branch, cumulative scope, actor, one surface, one operation, and no fallback; it does not bind or authorize an unknown PR, HEAD, or base. After activation, the consumer freshly derives those identities and may append exactly one exact assignment to the cumulative Canonical Task Issue body. The Task Issue URL is the assignment's direct `authority_source` and `canonical_record`; the pre-existing body must remain an exact prefix and the full patched resource must refetch byte-identically. Predelegation and exact assignment are distinct structural profiles with cardinalities `1` and `0..1`. A stale materialized assignment, duplicate, conflict, malformed record, drift, or ambiguous PATCH stops without another materialization or publication attempt.
