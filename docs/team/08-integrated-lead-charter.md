...
- terminal implementation completion with complete validation commits the exact validated tree once under the normal-execution predelegation and dispatches task-local prepublication Independent Review bound to that clean exact commit;
...
Normal-execution authority is the stable Product Owner `task_assignment` predelegation in the Canonical Task body, not the `wait_threads` cursor. It binds the exact repository, Task, initial fresh base, branch, registered worktree path, cumulative scope, actor, and `BOUNDED_EXECUTION_IDENTITY_V1`. It permits one exact validated-tree commit per newly admitted execution identity, one unchanged initial push plus non-Draft PR creation after prepublication approval, and one exact leased successor push to that existing PR per reviewed same-task correction identity. It does not permit scope expansion, rebase, amend, another PR, Ready mutation, Merge, retry, or Issue closure. Architecture or scope change and semantic base conflict remain Product Owner or Architect boundaries.
