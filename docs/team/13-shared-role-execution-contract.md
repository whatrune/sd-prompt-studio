...
The Integrated Lead remains the continuation owner after dispatching a Worker or reviewing Role. It MUST wait through the existing `wait_threads` target and cursor. A terminal response is applicable only when its cursor differs from the task-local consumed cursor; the coordinator advances the consumed cursor before dispatching the one action, and repeated delivery of that cursor performs no additional action. An applicable terminal event in the same coordination turn MUST fresh-bind the exact execution identity and perform exactly one deterministic next-stage action: canonical Task admission to exact worktree creation and implementation dispatch; implementation completion to one exact validated-tree commit plus task-local prepublication Independent Review; prepublication APPROVE to unchanged non-Draft publication; verified publication to current-HEAD check wait; checks PASS to Fresh Review; Review finding to the same owning Worker; correction completion to one new exact validated-delta commit for its execution identity; correction checks PASS to replacement Fresh Review; replacement Fresh Review APPROVE to exact corrected-thread resolution followed by canonical Review publication and read-only pre-Decision preflight; ordinary Fresh Review APPROVE directly to canonical Review publication and preflight; or successful preflight to transient `MERGE_READY` and STOP. The correction continuation carries the consumed finding cursor, finding HEAD, and sorted exact thread identities without turning them into durable authority. An active unresolved non-outdated Review thread discovered by any live-binding acquisition in that exact Review-authority/preflight operation, including assignment materialization, remains a failed preflight, but the runner projects its exact Task, PR, HEAD, scope, and sorted thread identities into one deterministic `REVIEW_FINDING` cursor for the same continuation owner. Before follow-up, the finding continuation MUST freshly observe and re-admit the exact Task, branch, registered worktree, local HEAD, remote base, PR generation, and authorized scope through `BOUNDED_EXECUTION_IDENTITY_V1`.

Timeout, nonterminal state, stale HEAD, identity mismatch, failed validation/checks, Architecture or scope change, semantic base conflict, and Fresh Review `CHANGES_REQUIRED` prohibit stage advance. The live-preflight thread projection MUST NOT reclassify those failures and supplies no authority of its own. A terminal event MUST NOT create Worker self-resumption, fuzzy Worker selection, automatic retry, a durable lifecycle state, or an intermediate passive wait. The event-to-next-action coordination target is at most 10 seconds, excluding external tool or service execution.

A Role execution that starts any Task-owned preview MUST invoke `scripts/run-task-owned-preview-v1.ps1` from its exact Task worktree and retain the helper invocation's exec-session and Windows Job Object identity as execution-local ownership. The helper creates the direct Vite process suspended, assigns it to the bounded Job Object before execution, and lets descendants inherit that same owner without OS-wide discovery. The owning execution requests teardown by sending exactly one `STOP` line through that same session; the helper terminates the Job Object and requires its active-process count to reach zero. Before returning any successful terminal Role result, including `completed` or `APPROVE`, that same owning execution MUST require the helper's terminal `process_tree_absent = true` result and the helper exec-session itself to be terminal; wrapper or PTY interruption alone is not teardown evidence. Because the exact owned job is then empty, none of its processes can retain the Task worktree as current directory or through an open handle. If the helper cannot establish Job Object ownership before process resume, terminate the owned job, or prove zero active processes, the Role MUST use the existing applicable `blocked` or `failed` terminal Result Handoff to report that exact teardown failure. This failure handoff is permitted without verified teardown, but preview ownership remains bound to the same execution identity and MUST NOT transfer to Integrated Lead or terminal cleanup or be deferred to post-Merge worktree cleanup. This rule does not authorize retry, fallback, daemon, scheduler, cleanup-side process ownership detection, or OS-wide process discovery and does not change cleanup-helper semantics, worktree safety checks, or branch/ref preservation.

Fresh Review is a mandatory orchestration step. For Review publication, its closed result is semantic input and the terminal cursor is a wake-up signal only; it is an optional diagnostic, not publication authority. `threadId` and terminal-turn identity are optional diagnostics too, and publication-time independent proof of the reviewer execution is not required. Authority is the stable Product Owner predelegation plus freshly admitted GitHub state. The predelegation explicitly grants two bounded operations after Fresh exact-current-HEAD `APPROVE / 0 / 0 / 0`: one V2 logical-assignment Issue comment and one canonical Review publication bound to the exact selected `TASK_ISSUE_COMMENT` or `PULL_REQUEST_REVIEW` surface, each with `operation_count: 1` and mandatory direct refetch. These are the complete normal Review-publication mutation budget; they require no additional Product Owner authorization and authorize neither Merge nor any alternate publication. Existing canonical predelegations with the prior single materialization count retain this same already-declared two-resource outcome through a closed legacy projection, so an in-flight Task does not require a body mutation. The publisher requires exact Task, PR, current HEAD/base/branch, cumulative scope, decision, `blocking / remaining / unknown`, required checks, zero active non-outdated threads, Merge applicability, actor, and surface admission; `APPROVE` requires exactly `0 / 0 / 0`. Missing, malformed, stale, ambiguous, nonzero-count, HEAD-mismatched, or live-state-mismatched input fails closed. The exact assignment logical identity is repository + Task + PR + HEAD + protected action + actor + surface + decision. Physical assignment comments with one logical identity and byte-identical canonical semantic payload are equivalent and collapse to one authority; conflicting payloads fail closed. Exact assignment and Review publication remain separate single-attempt mutations with direct refetch, no PATCH, no ambiguous-result retry, and no alternate-surface fallback. Consumed-event requirements for implementation commit, prepublication Review, and corrected-thread resolution remain mandatory.

The prepublication Independent Review is a task-local unchanged-publication gate bound to one clean exact commit. It is not the authoritative Simplified V1 Review and cannot authorize Merge. The protected host MUST refetch the exact consumed Review event and MUST NOT accept a caller-constructed decision object. Publication requires separate explicit authority and MUST push exactly that reviewed commit without amend, rebase, additional commit, or tree change before creating one non-Draft PR. Current-main, identity, scope, or byte drift stops before publication. The post-publication Fresh exact-HEAD Review remains the only authoritative Merge Review.

For normal Task execution, separate explicit authority is one stable Product Owner `task_assignment` predelegation embedded in the Canonical Task body. It binds repository, Task, exact initial base, branch, canonical worktree path, cumulative scope, actor, and `BOUNDED_EXECUTION_IDENTITY_V1`; admits exact registered-worktree creation once; admits one initial commit whose changed paths equal cumulative scope; admits one exact non-empty correction-delta commit per distinct correction execution identity only when every delta path is within that cumulative scope; admits the unchanged reviewed commit's initial push plus one non-Draft PR creation; admits one exact leased successor push to that existing PR per reviewed same-task correction identity; admits one resolution per exact still-active corrected thread after replacement Fresh Review approval; and may predelegate one exact post-terminal-cleanup Canonical Task closure under the policy described below. Each operation fresh-rechecks the applicable Task, base, branch, worktree, local HEAD, existing remote PR HEAD, cumulative scope, exact delta where applicable, actor, and remote resource immediately before mutation, then refetches the exact consumed terminal event from one absolute host-owned temporary UTF-8 JSON file outside the operation request. The production host rejects a missing file or target/cursor mismatch and fetches the exact expected PR before execution admission. Commit requires the event's complete implementation Result Handoff with PASS validation and exact delta; unchanged or successor branch publication requires its prepublication Independent Reviewer `APPROVE / 0 / 0 / 0`. Caller-supplied substitutes fail closed for those normal-execution operations. The initial base is the authority anchor: a descendant current `origin/main` with no authorized-path overlap may rebind the compatibility-validation base, while the Task branch remains on the existing PR lineage; divergence or overlap requires compatibility reconciliation. Correction publication proves the leased remote HEAD is the reviewed local HEAD's parent before its one push, performs no PR-create mutation, and directly refetches the existing PR at the successor HEAD. Corrected-thread resolution requires the host-refetched consumed finding event to reproduce the finding cursor, finding HEAD, exact sorted thread identities, Task, PR, base, branch, and cumulative scope before any resolution mutation. A new predelegation must explicitly grant each actual thread-resolution mutation; a compatible legacy Task with no such grant may continue only when every listed thread is already resolved or outdated and therefore requires zero mutation. Any additional active thread returns to the same correction route. The predelegation does not authorize rebase, amend, scope expansion, arbitrary thread resolution, another PR, Ready, Merge, or retry. A cursor activates at most one operation but is never its authority.

The normal-execution predelegation's exact Task-closure policy is either `AUTO_CLOSE_COMPLETED` or `KEEP_OPEN`. Ordinary newly serialized Canonical Tasks default to `AUTO_CLOSE_COMPLETED` unless `KEEP_OPEN` is explicit; a legacy Task with no closure operation or policy is interpreted as `KEEP_OPEN`. The closure operation activates only after post-Merge verification PASS, local-main synchronization PASS, and terminal Task-worktree cleanup PASS. It binds the exact repository, canonical Task Issue, merged PR and HEAD, branch, captured worktree path, and the returned synchronization and cleanup evidence. An already-closed Task or `KEEP_OPEN` is zero-mutation PASS. `AUTO_CLOSE_COMPLETED` permits exactly one close of only that Canonical Task Issue with state reason `completed`, requires byte-identical preservation of the complete Issue body, and directly refetches and verifies the closed resource. There is one Issue-close mutation transport, no fallback surface, and no retry after an ambiguous result. This predelegation is not inferred from the Product Owner Merge Decision, and close failure cannot invalidate the confirmed Merge, successful synchronization, or successful cleanup.
...
Canonical Task Issue creation uses one Node-owned composite serializer and one narrowly bounded server-assigned-number self-binding PATCH. The CREATE body is a structurally validated but non-authoritative `UNBOUND_CREATE` representation with reserved Task number `0`. After CREATE and direct refetch equality, `BOUND_FINAL` derives the Task Authority number and both predelegations' Task-number fields from the single returned Issue number. Only Task Authority `task_issue` and each predelegation's `task_id`, `authority_source`, `canonical_record`, and `allowed_changes.task_issue` may change. The complete Markdown, exact sorted scope, all other authority semantics, fences, encoding, and newline policy MUST remain stable. CREATE and PATCH are each attempted at most once, use the same UTF-8 file-backed serializer output, and require direct refetch equality; ambiguity stops without retry or recovery reconstruction. The final Issue is canonical only after exactly one Task Authority, one normal-execution predelegation, and one Review-publication predelegation parse and self-bind to the assigned number. This exception does not permit PATCHing Review, Merge Decision, derived exact-assignment, or other protected-action records.
...
After Merge and post-Merge verification succeed, the terminal owner runs local-main synchronization and Task-worktree cleanup under their existing contracts. Only when both return PASS does it evaluate the exact Canonical Task's predelegated closure policy. `AUTO_CLOSE_COMPLETED` closes the exact Task as completed; `KEEP_OPEN`, a missing legacy policy, or an already-closed Task performs no mutation. Closure is terminal housekeeping after the integration result and its failure is reported separately without changing the Merge, synchronization, or cleanup outcomes.
