...
- `current_pr_validation_republication` is a bounded validation owner for an existing canonical Draft PR whose Task-state still names prior HEAD A after another admitted protected owner published correction HEAD B. Its exact 17-field Product Owner authority binds the repository, Task, PR, A, B, current base, unchanged authorized paths, the existing correction authority and completion records, an ordered validation command list, and `operation_count = 1`. Admission requires the open unmerged Draft PR, unchanged base and branch, exact scope, a direct single-parent A → B commit, present-file blob identities, and a unique unconsumed authority. It cannot implement, commit, push, alter scope, or legitimize a manual or otherwise unowned HEAD change.
- The default-branch host checks out exact B only for the authority-bound validation commands, records each exit status and bounded output SHA-256, refetches HEAD/base/blob identities, and publishes one deterministic, self-bound `result_handoff` record with `validation_generation_sha256`. The existing Task-state writer then binds `observed_head` to B with unchanged paths and `review_status = PENDING`, and the existing Independent Reviewer dispatch is reused. Lifecycle validation may consume this record only for exact B when the original publication generation is `INCOMPLETE`; malformed, stale, mismatched, failed-command, or digest-invalid records fail closed. The owner does not perform Review, Ready, terminal observation, Merge Decision, or Merge.
- The current PTA manifest contains six hosted job identities after adding this bounded operator. Historical lifecycle evidence created when the manifest contained five identities remains validated against the frozen historical five-job roster; current evidence must bind the current six-job roster.
