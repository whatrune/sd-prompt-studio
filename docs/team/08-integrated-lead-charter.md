...
- an active unresolved non-outdated Review thread discovered by that exact live preflight keeps the preflight failed and projects one deterministic exact Task/PR/HEAD/thread-bound `REVIEW_FINDING` cursor back to the same owning Worker; and
