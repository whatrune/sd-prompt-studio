...
同じApplication Shellを共有するが、Route配下のDomain、State、Feature、API Clientを分離する。
...
`/research`の直接読込には、Local Companion ServiceがVite SPAのfallbackを提供する必要がある。現在のCompanion Service文書はFrontend buildの配信を定義するが、`/research` deep linkのfallback契約を明示していない。Frontend側でRouteを`/#/research`へ変更して回避せず、実装前にBackend Issueとして確認する。
...
- Receipts
- Validation Results
- Other indexed Research Artifacts

Derived Indexの`artifact_type`を正本とし、未知のtypeを隠さずOtherとして表示する。Frontendが新しいArtifact分類を生成しない。
...
| Format | 表示 |
|---|---|
| JSON | Parsed tree、syntax highlight、raw text、copy |
| YAML | Parsed tree、syntax highlight、raw text、copy |
| Markdown | rendered preview、raw text、copy |
| Plain Text | raw text、line表示、copy |
...
## 19. Backend / API Gap and Issue Candidates

Frontend都合で契約を変更せず、次を実装前Issueとして整理する。

### 19.1 `/research` SPA fallback

Local Companion Serviceが`/research`の直接読込を`index.html`へfallbackする契約が明文化されていない。Route要件を維持したまま、Companion側のFrontend配信契約として確認が必要である。

### 19.2 Artifact単位Schema Version
...
### 19.3 Research/Audit Hash Algorithm Metadata

`research_audit_hashes`は`name`、`value`、`source`を持つが、algorithm Fieldを持たない。Frontendでcontract semanticsを複製せず、exact contract nameを表示する。Algorithmの構造表示が必須ならSchema/API Issueとする。

### 19.4 Stale後のIndex更新

`ARTIFACT_STALE`を返した後に、同一Service process内でIndexを再生成するread-only endpointはない。Frontendは無限retryまたはfilesystem fallbackを実装せず、運用上の再生成・再起動案内を表示する。自動refreshが必要ならCompanion Service Issueとする。
