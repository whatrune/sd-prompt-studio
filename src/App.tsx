...
import { compatibilityLabel, generationNote, modelHints } from './engine/tagIntelligence'
...
export default function App() {
  ...
  return <main className="app-shell">
    ...
    {analyzerOpen&&<div className="modal-backdrop" onMouseDown={()=>setAnalyzerOpen(false)}><section className="analyzer-modal" onMouseDown={e=>e.stopPropagation()}><div className="analyzer-head"><div><span className="eyebrow">PROMPT ANALYZER</span><h2>既存プロンプトをGUIへ取り込む</h2></div><button onClick={()=>setAnalyzerOpen(false)}><X size={18}/></button></div><p>カンマ、改行、角括弧、BREAKを解析し、辞書一致またはキーワード推定でカテゴリ分けします。</p><textarea value={analyzerText} onChange={e=>setAnalyzerText(e.target.value)} placeholder="masterpiece, 1girl, blue hair, ..."/><div className="analyzer-preview">{analyzerEntries.map((entry,i)=><span key={`${entry.source}-${i}`}><b>{categoryLabels[entry.category]||entry.category}</b>{entry.prompt}{entry.matched?'':'（推定）'}</span>)}</div><div className="modal-actions"><button className="ghost" onClick={()=>setAnalyzerText('')}>クリア</button><button onClick={()=>{analyzerEntries.forEach(entry=>store.addTag({...entry.dictionaryTag,id:entry.dictionaryTag?.id||`analyzed-${createId()}`,prompt:entry.prompt,label:entry.label,category:entry.category,subcategory:entry.subcategory,weight:entry.weight}));setAnalyzerOpen(false)}}><BookOpen size={16}/>解析結果を追加</button></div></section></div>}
  </main>
}
