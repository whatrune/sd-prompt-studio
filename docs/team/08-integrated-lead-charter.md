...
Review authority publication uses the existing closed compatibility contract and remains a separate protected action. Integrated Lead MUST inspect both surfaces first and reuse exactly one byte-identical current-HEAD authority with zero mutation. When no Review exists, semantic `REVIEW_APPROVE` is only an activation prerequisite. Integrated Lead reuses one exact Task-body assignment when present; otherwise it requires exactly one Product Owner materialization-only predelegation whose existing `task_assignment` fields bind `REVIEW_AUTHORITY_PUBLICATION`, `FRESH_EXACT_HEAD_REVIEW_APPROVE`, repository, Task, branch, scope, authenticated actor, one surface, `APPROVE / 0 / 0 / 0`, `operation_count = 1`, and no fallback. After fresh PR, HEAD, and base acquisition, it may PATCH the Canonical Task body exactly once to append one deterministic exact assignment while preserving the existing Task Authority and predelegation bytes. The Task Issue URL remains `authority_source` and `canonical_record`. Integrated Lead MUST refetch the full body and repeat live HEAD, base, actor, assignment, and surface checks immediately before Review mutation. Duplicate, conflicting, malformed, stale, drifted, or ambiguous authority stops without retry. A distinct actor may use only an assignment-authorized exact-commit PR `APPROVE`; a self actor may use only an assignment-authorized identical top-level canonical Task Issue comment. An arbitrary PR Review HTTP, permission, or transport failure MUST NOT trigger the compatibility route.
