...
- A terminal implementation Result Handoff with complete validation MUST use the Canonical Task's normal-execution predelegation to commit the exact validated scope once for that execution identity, then immediately dispatch prepublication Independent Review. That review is a task-local unchanged-publication gate, not the authoritative Merge Review. Its approval MUST bind the clean exact commit; publication MUST push that exact commit without amend, rebase, additional commit, or tree change and MUST create one non-Draft PR through the same closed predelegated route.
