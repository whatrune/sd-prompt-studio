...
try {
  ...
  assert(appSource.includes('className="saved-prompt-asset-main" onClick={()=>setPendingApplyPrompt(saved)}'), 'Saved Prompt card bodies must open only the apply dialog')
  assert(appSource.includes('className="saved-prompt-info"') && appSource.includes('onClick={()=>setSelectedSavedPrompt(saved)}'), 'Saved Prompt detail controls must update only the Inspector selection')
