...
try {
  ...
  const matchedCombination = appSource.indexOf('className="visual-concept-risk-matched-combination"')
  const modelBoundary = appSource.indexOf('className="visual-concept-risk-model-boundary"')
  check(matchedCombination > riskAnnouncement && matchedCombination < evidenceDetails && appSource.includes("[...entry.trigger_context.required_prompt_tags,...entry.trigger_context.trigger_prompt_tags].map(tag=>tag.prompt).join(' + ')") && appSource.includes('Matched combination:'), 'the active upper-body advisory must show the already-projected matched combination as friendly prompt text')
  check(modelBoundary > informationalAssurance && modelBoundary < evidenceDetails && appSource.includes('This advisory reflects the evaluated model and matched prompt context only.'), 'a short model-bounded explanation must precede collapsed technical evidence without exposing internal model identity')
  ...
  check(appSource.includes('const visualConceptVisibilityRiskEntries = visualConceptRiskEntries.filter(entry=>entry.trigger_context.required_visible_region_concept_ids.length>0)') && appSource.includes('const requiredVisibleRegionLabels = requiredVisibleRegionConceptIds.map(conceptId=>VISUAL_CONCEPT_VISIBILITY_LABEL_V1[conceptId])') && appSource.includes('const visibilityIntentAcknowledgement = requiredVisibleRegionConceptIds.length > 0 && visualConceptVisibilityRiskEntries.length === 0') && appSource.includes('No known risk affecting these required regions for the current selection.') && acknowledgementRender > 0 && acknowledgementRender < mappedEntryBranch, 'active visibility intent without a known visibility risk must remain explicitly scoped to the named required visible regions when an unrelated advisory category is present')
  ...
  check(stylesSource.includes('.visual-concept-advisory-entry') && stylesSource.includes('.visual-concept-advisory-coverage') && stylesSource.includes('.visual-concept-advisory-uncovered') && stylesSource.includes('.visual-concept-risk-advisory') && stylesSource.includes('.inspector-risk-indicator') && stylesSource.includes('.visual-concept-risk-model-boundary'), 'advisory must have deterministic Inspector-native visual treatment')
