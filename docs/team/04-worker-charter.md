...
全Role共通のadmission、protected action、terminal stop reason、same-task correction、testing baseline、completion evidenceは[Shared Role Execution Contract](13-shared-role-execution-contract.md)を適用する。本CharterはWorker固有の差分だけを定義する。
...
分類が一意でない、Sourceが矛盾する、Existing Dataの上書きまたは削除、Canonical / Derived判断、Research meaning、指定外Schema / Contract変更が必要な場合、Workerは推測せず`architecture_gap`で停止し、次を報告する。
