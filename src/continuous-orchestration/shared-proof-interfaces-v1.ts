import type { AutomaticGateProgressionEvaluationResultV2 } from '../automatic-gate-progression'
import {
...
const mapAgp=(source:AutomaticGateProgressionEvaluationResultV2,noTransitionBinding?:Readonly<{future_event_type:string;future_event_role_id:string}>):ProgressionEvaluatorResultV1|null=>{switch(source.kind){case'recommend_next_role':return{kind:'recommend_next_role',target_role_id:source.target_role,next_action_id:source.next_action,predecessor_canonical_url:source.predecessor_canonical_url,target_head_sha_or_null:source.target_head};case'wait_for_protected_action':return{kind:'wait_for_protected_action',protected_action_id:source.protected_action};case'require_gate_status_update':return{kind:'require_gate_status_update'};case'invalidate_approval':return{kind:'invalidate_approval',invalidation_class:source.invalidation_reason};case'stop':return{kind:'stop',stop_condition:source.stop_condition==='blocking_finding_recurrence'?'blocking_finding':source.stop_condition==='canonical_conflict'?'canonical_conflict':source.stop_condition==='ambiguous_role_ownership'?'ambiguous_role_ownership':source.execution_stop_reason,recovery_role_id:source.recovery_owner};case'no_transition':return noTransitionBinding?{kind:'no_transition',future_event_type:noTransitionBinding.future_event_type as never,future_event_role_id:noTransitionBinding.future_event_role_id}:null}}
export const deriveProgressionDecisionPortShadowV1=(source:AutomaticGateProgressionEvaluationResultV2,noTransitionBinding?:Readonly<{future_event_type:string;future_event_role_id:string}>):ClosedAdmissionResultV1<ProgressionDecisionPortV1>=>{try{const projected_result=mapAgp(source,noTransitionBinding);if(!projected_result)return rejected({code:'invalid_cross_input_binding',path:'/progression_port/no_transition_binding'});const base={progression_decision_port_version:PROGRESSION_DECISION_PORT_V1_VERSION,source_result_kind:source.kind,source_result_digest:digest(source),projected_result,shadow_only:true as const,transport_invoked:false as const};return validateProgressionDecisionPortV1(seal(base as unknown as JsonObject,'port_digest'))}catch{return failed('progression_port_adapter')}}
