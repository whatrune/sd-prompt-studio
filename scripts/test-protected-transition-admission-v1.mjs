...
const providerUnits = [
  ...
  },
  {
    ...
    evidence: [
      ...
      providerExec.provider_projection.exec_argv.join('|') === `exec|--ignore-user-config|--sandbox|workspace-write|--ephemeral|--json|--cd|${WINDOWS_WORKSPACE}|-`,
...
const hostRunnerBindingMatrix = [
  ...
  providerExecutionStep?.run.includes('$prompt | & codex.cmd exec --ignore-user-config --sandbox workspace-write --ephemeral --json --cd $env:GITHUB_WORKSPACE -') && workflowSource.split('& codex.cmd exec').length === 2,
