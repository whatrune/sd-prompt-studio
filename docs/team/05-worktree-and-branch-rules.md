...
同helperはTask checkoutを変更する前に、exact repositoryからabsolute Git common-directoryを解決し、その`worktrees` administration rootで同一processによる一意なprobe directoryとdelete-on-close fileのcreate、flush、close、directory delete、およびabsence verificationを完了しなければならない。このprobeはGit refまたはTask worktree metadataではなく、成功時にresidueを残さない。fileとdirectoryのcreate/delete capabilityを証明できないhostでは`worktree_cleanup_git_common_dir_not_writable`としてfail closedにし、`git worktree remove`を呼ばない。別hostへのfallback、self-elevation、prune、force、またはretryは行わない。
