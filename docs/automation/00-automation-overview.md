...
| Current PR Validation Republication V1 | Protected Transition Admission revalidates one already-published open Draft PR after a canonically completed same-scope correction changed its HEAD, then publishes a self-bound Result Handoff and returns the existing Task-state and reviewer path to the new exact HEAD | bounded `workflow_dispatch.current_pr_validation_republication` route | repository-reachable after the implementing exact HEAD reaches the default branch |
...
- `current_pr_validation_republication` is a bounded validation owner for an existing canonical Draft PR whose Task-state still names prior HEAD A after another admitted protected owner published correction HEAD B. Its exact 17-field Product Owner authority binds the repository, Task, PR, A, B, current base, unchanged authorized paths, the existing correction authority and completion records, an ordered list of closed versioned validation command IDs, and `operation_count = 1`. Admission requires the open unmerged Draft PR, unchanged base and branch, exact scope, a direct single-parent A → B commit, present-file blob identities, and a unique unconsumed authority. The default-branch runner alone maps admitted IDs to frozen executable/argument arrays; arbitrary command text, unknown IDs, duplicate IDs, and shell evaluation of authority data fail closed. The owner cannot implement, commit, push, alter scope, or legitimize a manual or otherwise unowned HEAD change.
- Exact B is checked out with `persist-credentials: false` into a disposable `windows-latest` validation job whose only permission is `contents: read`. Locked dependencies are installed without lifecycle scripts, and every closed-catalog subprocess runs without `GH_TOKEN`, `GITHUB_TOKEN`, or GitHub workflow command-file variables. This job transports only bounded command IDs, exit statuses, output SHA-256 digests, and exact HEAD/base/blob metadata; raw command output is not passed to the protected owner.
- A separate write-capable `ubuntu-latest` finalizer checks out only the exact default-branch workflow SHA and executes no code from B. It freshly reacquires the open Draft PR, exact B and base, branch, commit/blob scope, Task-state, authority, and correction chain before accepting the bounded validation metadata and publishing one deterministic, self-bound `result_handoff` record with the existing `validation_generation_sha256` semantics. The existing Task-state writer then binds `observed_head` to B with unchanged paths and `review_status = PENDING`, and the existing Independent Reviewer dispatch is reused. Lifecycle validation may consume this record only for exact B when the original publication generation is `INCOMPLETE`; malformed, stale, mismatched, failed-command, or digest-invalid records fail closed. Neither validation nor finalization performs Review, Ready, terminal observation, Merge Decision, or Merge.
- The current PTA manifest contains seven hosted job identities after separating disposable validation from trusted finalization. Historical lifecycle evidence created when the manifest contained five identities remains validated against the frozen historical five-job roster; current evidence must bind the current seven-job roster.
