...
try {
  ...
  const favoriteTestId = tags[0].id
  usePromptStore.getState().toggleFavorite(favoriteTestId)
  assert.equal(usePromptStore.getState().favoriteIds.includes(favoriteTestId), true, 'tag favorites must be stored by id')
  usePromptStore.getState().toggleFavorite(favoriteTestId)
  assert.equal(usePromptStore.getState().favoriteIds.includes(favoriteTestId), false, 'tag favorites must be removable')
  ...
  const promptWorkspaceSource = appSource.slice(appSource.indexOf('<div className="prompt-workspace-content">'), appSource.indexOf('<aside className="preview panel">'))
  assert(promptWorkspaceSource.indexOf('className="composer-placeholder"') < promptWorkspaceSource.indexOf('className="tag-list-section"'), 'Composer extension slot must render before the Composer-owned Tag List')
  ...
  assert(appSource.includes('function navigateToFavorites()'), 'Favorite navigation must have an explicit Workspace action')
  ...
  assert(appSource.includes('!isSearchMode&&favoritesOnly&&<section className="category-tabs-section" aria-label="お気に入りカテゴリ"'), 'Favorite Mode must always render the all-favorites tab, including when no favorites exist')
  ...
  assert(appSource.includes('aria-label="検索をクリア"'), 'Search clear button must have an accessible name')
  ...
  assert(appSource.includes("const [activeLibraryView, setActiveLibraryView] = useState<'saved-prompts' | 'user-dictionary'>('saved-prompts')"), 'User Dictionary browsing must use non-persisted local Library view state')
  assert(appSource.includes('function navigateToUserDictionary()'), 'Library navigation must expose an explicit User Dictionary action')
  assert(appSource.includes('<span className="navigation-item-label">User Dictionary</span><small>{store.userTags.length}</small>'), 'Library navigation must expose User Dictionary with the persisted entry count')
  ...
  assert(userDictionaryWorkspaceSource.includes('ユーザー辞書に登録されたタグはありません'), 'User Dictionary must render a clear empty state')
  assert(appSource.includes("const [userDictionaryRegistrationCategory, setUserDictionaryRegistrationCategory] = useState('')"), 'User Dictionary registration must not silently default to Quality or another category')
  assert(userDictionaryWorkspaceSource.includes('className="user-dictionary-registration"'), 'User Dictionary Library must own the registration surface')
  assert(userDictionaryWorkspaceSource.includes('<option value="">カテゴリを選択</option>') && userDictionaryWorkspaceSource.includes('categoryOrder.map(categoryKey=><option'), 'registration must offer only the canonical production category registry after an explicit empty choice')
  assert(userDictionaryWorkspaceSource.includes('{getCategoryLabel(categoryKey,locale)}'), 'registration categories must use the existing localized product labels')
  assert(appSource.includes('if (!customPrompt.trim() || !categoryOrder.includes(userDictionaryRegistrationCategory)) return'), 'registration must reject missing or non-production categories')
  assert(appSource.includes('store.addCustomTag(customPrompt, userDictionaryRegistrationCategory, true, customLabel)'), 'registration must preserve canonical addCustomTag ownership and persist the chosen category')
  assert(appSource.includes("setUserDictionaryRegistrationCategory('')"), 'registration must require an explicit category choice for every new entry')
  assert(appSource.includes('{isUser&&<button className="delete-user-tag"') && appSource.includes('onClick={()=>store.removeUserTag(tag.id)}'), 'User Dictionary deletion must reuse the canonical removeUserTag owner without changing shared cards')
  const settingsDictionarySource = appSource.slice(appSource.indexOf('<section className="settings-dictionary">'), appSource.indexOf('</section>', appSource.indexOf('<section className="settings-dictionary">')))
  assert(settingsDictionarySource.includes('exportUserDictionary') && settingsDictionarySource.includes('importUserDictionary'), 'Settings must retain compatible bulk Import and Export')
  assert.equal(settingsDictionarySource.includes('settings-custom-form') || settingsDictionarySource.includes('settings-user-list') || settingsDictionarySource.includes('clearUserTags'), false, 'Settings must not retain registration, inline entry management, per-entry deletion, or bulk clear')
  assert(appSource.includes("{store.workspaceView==='library'&&!userDictionaryOnly?<>"), 'User Dictionary usage must show the normal current-prompt Inspector instead of Saved Prompt details')
  const toggleDictionaryTagSource = appSource.slice(appSource.indexOf('function toggleDictionaryTag(tag: PromptTag)'), appSource.indexOf('function handleTagSearchKeyDown'))
  assert(toggleDictionaryTagSource.includes('store.addTag({...tag, weight: 1})') && toggleDictionaryTagSource.includes('store.removeTagFromLayer(layerId, selected.id)'), 'User Dictionary click and toggle must converge on the canonical tag add/remove path')
  ...
  assert.equal((appSource.match(/aria-label="Promptを保存" title="Promptを保存"/g)??[]).length, 2, 'Prompt Actions save controls must be icon-only buttons with accessible labels and tooltips')
  assert.equal((appSource.match(/aria-label="Promptをクリア" title="Promptをクリア"/g)??[]).length, 2, 'Prompt Actions clear controls must be icon-only buttons with accessible labels and tooltips')
  ...
  assert(stylesSource.includes('scrollbar-gutter: auto') && stylesSource.includes('padding-inline: var(--pane-control-inset)'), 'Header actions must align with the Inspector control inset without positional offsets')
