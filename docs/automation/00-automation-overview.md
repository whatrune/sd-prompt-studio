...
| Pre-PR Implementation Authority Resume V1 | Protected Transition Admission fresh-fetches one finalized self-bound 21-field authority by explicit comment ID and reuses the existing Worker dispatch | bounded `workflow_dispatch.pre_pr_implementation_authority_resume` route | repository-reachable after the implementing exact HEAD reaches the default branch |
...
- `pre_pr_implementation_authority_resume` is a bounded resume route for a finalized self-bound `pre_pr_implementation_authority_v1` whose creation event carried a placeholder body. A new explicit dispatch identifies the authority comment, Task, exact baseline, branch, worktree, and `operation_count = 1`; the default-branch owner fresh-fetches the comment, authenticates the Product Owner and open Task, requires the exact production 21-field schema and self URL, scans canonical Task comment history for uniqueness and prior Result Handoff consumption, binds the fetched body SHA-256, and then emits the existing `IMPLEMENTER` dispatch. The existing Worker, host rebind, validation, and `pre_pr_implementation_result_handoff_v1` publication remain unchanged.
- Resume V1 does not modify or reinterpret `issue_comment.created`, does not subscribe to edited comments, and does not accept the unmerged 23-field `EXACT_AUTHORIZED_DELTA` form. Any 23-field pre-PR authority is explicitly unsupported and stops before Worker dispatch. Placeholder, wrong actor, wrong Task/repository, stale base, wrong branch/worktree, malformed self-binding, duplicate history identity, or an authority already referenced by a canonical Result Handoff also stops without mutation or retry.
