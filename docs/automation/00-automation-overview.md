...
The closed Review publication route fetches the authenticated actor and PR author before mutation. A distinct actor uses the normal PR `APPROVE` endpoint; a self actor directly uses `TASK_ISSUE_COMMENT` because GitHub prohibits PR self-approval. Before either mutation it enumerates both canonical surfaces for the exact Task, PR, and HEAD. Exactly one byte-identical authority is reused with zero mutation; duplicates, conflicts, malformed records, or live binding drift fail closed. A created resource is refetched and required to preserve body, parser, resource, actor, and exact-HEAD identity. The Task-comment route is never a fallback for an arbitrary PR Review API failure.
