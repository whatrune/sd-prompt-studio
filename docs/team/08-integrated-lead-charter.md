...
- correction checks PASS dispatches replacement Fresh Review while preserving the consumed finding cursor, finding HEAD, and exact corrected-thread identities; and
- terminal replacement Fresh Review `APPROVE / 0 / 0 / 0` resolves each exact still-active corrected thread once under the normal-execution predelegation before ensuring exactly one canonical current-HEAD Review authority and running the read-only pre-Decision preflight; an ordinary Fresh approval has no thread-resolution step; and
...
Review authority publication uses the existing closed compatibility contract and remains a separate protected action. Fresh Review is a mandatory orchestration step, and its closed result is semantic input to publication. `wait_threads` supplies only a wake-up cursor; it is not publication authority. `threadId`, terminal-turn identity, and cursor are optional diagnostics and do not require publication-time proof of the reviewer execution. Integrated Lead MUST inspect both Review surfaces first and reuse exactly one byte-identical current-HEAD authority with zero mutation. Otherwise, a stable Product Owner predelegation in the Canonical Task body and an exact Task/PR/current-HEAD `APPROVE / 0 / 0 / 0` semantic input activate deterministic assignment derivation only after fresh Task, PR, HEAD, base, branch, cumulative scope, checks, active-thread, mergeability, actor, and surface admission. Missing, malformed, stale, ambiguous, nonzero-count, HEAD-mismatched, or live-state-mismatched input fails closed.
...
Normal-execution authority is the stable Product Owner `task_assignment` predelegation in the Canonical Task body, not the `wait_threads` cursor. It binds the exact repository, Task, initial fresh base, branch, registered worktree path, cumulative scope, actor, and `BOUNDED_EXECUTION_IDENTITY_V1`. Each commit or publication activation must refetch the exact consumed target/cursor from the host-owned continuation-event file and admit its Result Handoff or Independent Review; request JSON cannot supply either decision. Production execution admission also fetches and binds the exact expected PR rather than relying on local worktree observation alone. The predelegation permits one initial full-scope validated-tree commit, then one exact non-empty correction-delta commit per newly admitted correction execution identity when that delta is a subset of cumulative authority; one unchanged initial push plus non-Draft PR creation after prepublication approval; one exact leased successor push to that existing PR per reviewed same-task correction identity; and one resolution per exact still-active corrected thread after replacement Fresh Review approval. The initial base remains the authority anchor; a freshly observed descendant `origin/main` can replace the compatibility-validation base only when the intervening path set is disjoint from the Task scope. The Task branch remains on the existing PR lineage, and publication must prove the leased remote HEAD is the reviewed local HEAD's parent before push. Divergence, overlap, an out-of-authority delta, or a non-descendant local HEAD stops without mutation. The predelegation does not permit scope expansion, arbitrary thread resolution, rebase, amend, another PR, Ready mutation, Merge, retry, or Issue closure. Architecture or scope change and semantic base conflict remain Product Owner or Architect boundaries.
