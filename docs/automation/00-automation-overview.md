...
After direct PR refetch binds the exact Task, branch, registered worktree, PR, base, HEAD, and authorized scope, the coordinator waits for current-HEAD checks. Checks PASS dispatches the authoritative Fresh exact-HEAD Review. Review findings stop Merge-ready progression and return only through the existing same-Worker correction route. Fresh Review `APPROVE / 0 / 0 / 0` projects `ENSURE_REVIEW_AUTHORITY_AND_RUN_PREFLIGHT`: the new opaque `wait_threads` cursor is consumed before advancement, an existing exact assignment or Review is reused when uniquely valid, otherwise one exact resource-bound assignment comment and the canonical current-HEAD Review are published, and the existing read-only pre-Decision preflight runs. Repeated delivery of the consumed cursor is `NO_ADVANCE`. A successful preflight projects transient `MERGE_READY` and stops for the Product Owner Merge Decision; it does not authorize Merge.
...
Task Authority, Review body, and Merge Decision use one Node-owned UTF-8 serializer/parser pair per record. A body must be complete at creation time, contain exactly one fenced JSON block, and round-trip byte-for-byte. When GitHub Pull Request Review publication is unavailable, the exact same Review body may be published once as a top-level comment on the canonical Task Issue and selected explicitly as `TASK_ISSUE_COMMENT`; it is one compatibility surface, not a second semantic decision. Placeholder-then-PATCH publication, Task-body exact-assignment materialization, PowerShell Markdown interpolation, self-referential comment URLs, and digest/seal layers are not part of V1.

The closed Review publication route fetches the authenticated actor and PR author and enumerates both canonical surfaces for the exact Task, PR, and HEAD before considering mutation. Exactly one byte-identical authority is reused with zero mutation. If no Review exists, the semantic Review result is not publication authority. The route requires exactly one stable Product Owner materialization-only predelegation in the Canonical Task body. After a new Fresh exact-HEAD `APPROVE / 0 / 0 / 0` terminal cursor is consumed, it freshly derives Task, PR, HEAD, base, branch, scope, actor, and surface and either reuses one applicable exact assignment or creates one top-level Task Issue comment. The deterministic assignment body uses `canonical_record: GITHUB_RESOURCE`; the successful CREATE response and direct refetch supply the canonical comment ID and URL. No Task-body PATCH or assignment-comment PATCH occurs. The runner requires exact response/refetch body, parser, actor, resource, cursor, and live-binding equality before Review publication. A distinct actor may use only the assignment-authorized PR `APPROVE` endpoint; a self actor may use only the assignment-authorized `TASK_ISSUE_COMMENT` surface because GitHub prohibits PR self-approval. Missing predelegation, stale current authority, duplicates, conflicts, malformed records, actor or live-binding drift fail closed. An ambiguous assignment or Review CREATE never switches surfaces or retries.
