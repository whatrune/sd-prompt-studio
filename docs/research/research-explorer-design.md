...
| Validation Result | Validation Run IDまたはReceipt ID | Validator JSON出力、Generation/Finalize ReceiptのValidation step | N:1 Artifact Snapshot | Contextと実行時刻 | `passed`、`failed`、`infrastructure_error` | Context、Exit Code、Error/Warning Count、Diagnostics |
...
- Candidate → Canonical Assertion: nested Assertion IDとFinalize Receipt
...
flowchart LR
    ...
    E --> F["Research Explorer Query Layer"]
    F --> G["Navigator / Viewer / Inspector"]
```
...
- Display用Metadata
- Source Content Hashまたは既存Identity Hash
- Relationship先ID
...
1. Research Workspace RouteがIndex Metadataを取得する。
2. Query LayerがEntityとRelationshipを正規化する。
3. NavigatorがIDとDisplay Metadataだけを読む。
4. Artifact選択時にViewerがSource Locatorから本文を取得する。
5. InspectorがIndex Metadata、Source Metadata、Receipt、Validation Resultを合成する。
6. Hash不一致や参照欠落はDiagnostic表示し、Source Artifactを修復しない。
...
- `ResearchIndexRepository`: Index Snapshot取得
- `ResearchArtifactRepository`: Source Artifact読込
- `ResearchExplorerQuery`: Entity検索とRelationship解決
- `ResearchExplorerUiState`: 選択・Filter・Layout

具体的なライブラリやファイル配置は実装PRで決定する。既存Zustand StoreへのField追加を本設計では要求しない。
...
- Derived Research IndexのFormat、生成CLI、鮮度Validation
...
| PR74 | Discovery Read Model | Index生成方式、鮮度検証、Repository Interface | Claim Mutation |
...
- Index生成をbuild時、CLI実行時、desktop serviceのどこで行うか
- BrowserからSource Artifactを取得するtransport
