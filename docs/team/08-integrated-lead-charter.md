...
Review authority publication uses the existing closed compatibility contract and remains a separate protected action. Integrated Lead MUST inspect both surfaces first and reuse exactly one byte-identical current-HEAD authority with zero mutation. When no Review exists, semantic `REVIEW_APPROVE` is only an activation prerequisite. Integrated Lead requires the new unconsumed Fresh Review `wait_threads` terminal cursor and advances its consumed cursor before any protected action; repeat delivery is `NO_ADVANCE`. It then reuses one applicable exact assignment or requires exactly one stable Product Owner materialization-only predelegation in the Task body. Under that predelegation it may create exactly one deterministic top-level Task Issue comment binding `REVIEW_AUTHORITY_PUBLICATION` to the exact repository, Task, PR, HEAD, branch, base, scope, authenticated actor, one permitted surface, closed Review body, predelegation identity, cursor, and `operation_count = 1`. The assignment body uses `canonical_record: GITHUB_RESOURCE`; the successful CREATE response and direct refetch supply its admitted comment identity. The Task body and assignment comment are never PATCHed. A distinct actor may use only an assignment-authorized exact-commit PR `APPROVE`; a self actor may use only an assignment-authorized identical top-level canonical Task Issue comment. Integrated Lead MUST repeat live identity, assignment, and surface checks immediately before Review publication, reject duplicate or conflicting authority, and refetch the selected resource before preflight. Missing or mismatched publication authority is a zero-Review-mutation stop. An arbitrary assignment or Review HTTP, permission, or transport failure MUST NOT trigger fallback or retry.
