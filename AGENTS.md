...
- For an already completed post-publication Fresh Review, `threadId`, terminal-turn identity, and `wait_threads` cursor are continuation diagnostics only. Their absence or later unavailability MUST NOT block canonical Review publication when the coordinator has the completed Fresh Reviewer execution bound to the exact Task, PR, and current HEAD with `APPROVE / 0 / 0 / 0`; the protected publisher still freshly requires the exact Task/PR/HEAD/base/branch/scope, required checks, zero active unresolved non-outdated threads, mergeability, actor, surface, direct publication refetch, and preflight. This does not admit an unfinished Review or caller-authored substitute and does not relax the consumed-event requirements for implementation commit, prepublication Review, or corrected-thread resolution.
- A terminal implementation Result Handoff with complete validation MUST be refetched from the exact consumed continuation target/cursor and bound to Task, branch, worktree, base, HEAD, execution identity, and scope before the Canonical Task's normal-execution predelegation may commit the exact validated scope once. For `COMMIT_VALIDATED_TREE`, the Implementer MUST emit the canonical machine object projected by `scripts/run-bootstrap-publication-operator-v1.mjs`; that same exported owner validates it at commit admission, without Role-name translation or compatibility guessing. Caller-supplied handoff or Review fields are not evidence. The subsequent host-refetched prepublication Independent Review is a task-local unchanged-publication gate, not the authoritative Merge Review. Its approval MUST bind the clean exact commit; publication MUST push that exact commit without amend, rebase, additional commit, or tree change and MUST create one non-Draft PR through the same closed predelegated route.
