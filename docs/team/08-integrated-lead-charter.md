...
Review authority publication uses the existing closed compatibility contract and remains a separate protected action. `wait_threads` supplies only a wake-up cursor; it is not publication authority. When a post-publication Fresh Reviewer execution has already completed against the exact Task, PR, and current HEAD with `APPROVE / 0 / 0 / 0`, its `threadId`, terminal-turn identity, and cursor are optional diagnostics and are not publication predicates; loss or non-materialization of those diagnostics does not require another Review. The completed semantic result itself must still come from that Fresh Reviewer execution rather than caller-authored prose. Integrated Lead MUST inspect both Review surfaces first and reuse exactly one byte-identical current-HEAD authority with zero mutation. Otherwise, a stable Product Owner predelegation in the Canonical Task body and the completed Fresh exact-HEAD result activate deterministic assignment derivation only after fresh Task, PR, HEAD, base, branch, cumulative scope, checks, active-thread, mergeability, actor, and surface admission.
