...
try {
  ...
  assert.equal(userDictionaryWorkspaceSource.includes('removeUserTag'), false, 'User Dictionary usage cards must not add a management/delete path')
  assert(appSource.includes("{isUser&&!userDictionaryOnly&&<button className=\"delete-user-tag\""), 'existing user-tag management affordances outside the dedicated usage surface must remain unchanged')
