...
- Fresh Review remains a mandatory orchestration step. After it completes, its closed exact-Task/PR/current-HEAD semantic result is publication input; `threadId`, terminal-turn identity, and `wait_threads` cursor are optional continuation diagnostics and are not publication authority. Publication-time independent proof of the reviewer execution is not required. The protected publisher MUST still validate the exact Task/PR/HEAD/base/branch/scope, decision, `blocking / remaining / unknown`, required checks, zero active unresolved non-outdated threads, mergeability, actor, surface, direct publication refetch, and preflight against the existing Product Owner predelegation. Missing, malformed, stale, ambiguous, nonzero-count, HEAD-mismatched, or live-state-mismatched input fails closed. This does not relax the consumed-event requirements for implementation commit, prepublication Review, or corrected-thread resolution.
- A terminal implementation Result Handoff with complete validation MUST be refetched from the exact consumed continuation target/cursor and bound to Task, branch, worktree, base, HEAD, execution identity, and scope before the Canonical Task's normal-execution predelegation may commit the exact validated scope once. For `COMMIT_VALIDATED_TREE`, the Implementer MUST emit the canonical machine object projected by `scripts/run-bootstrap-publication-operator-v1.mjs`; that same exported owner validates it at commit admission, without Role-name translation or compatibility guessing. Caller-supplied handoff or prepublication Review fields are not evidence for those normal-execution operations. The subsequent host-refetched prepublication Independent Review is a task-local unchanged-publication gate, not the authoritative Merge Review. Its approval MUST bind the clean exact commit; publication MUST push that exact commit without amend, rebase, additional commit, or tree change and MUST create one non-Draft PR through the same closed predelegated route.
