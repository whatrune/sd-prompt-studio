...
For normal Task execution, separate explicit authority is one stable Product Owner `task_assignment` predelegation embedded in the Canonical Task body. It binds repository, Task, exact initial base, branch, canonical worktree path, cumulative scope, actor, and `BOUNDED_EXECUTION_IDENTITY_V1`; admits exact registered-worktree creation once; admits one exact validated-tree commit per distinct execution identity; admits the unchanged reviewed commit's initial push plus one non-Draft PR creation; and admits one exact leased successor push to that existing PR per reviewed same-task correction identity. Each operation fresh-rechecks the applicable Task, base, branch, worktree, local HEAD, existing remote PR HEAD, scope, actor, and remote resource immediately before mutation, then refetches the exact consumed terminal event from one absolute host-owned temporary UTF-8 JSON file outside the operation request. The production host rejects a missing file or target/cursor mismatch and fetches the exact expected PR before execution admission. Commit requires the event's complete implementation Result Handoff with PASS validation; publication requires its Independent Reviewer `APPROVE / 0 / 0 / 0`. Caller-supplied substitutes fail closed. The initial base is the authority anchor: a descendant current `origin/main` with no authorized-path overlap may rebind the compatibility-validation base, while the Task branch remains on the existing PR lineage; divergence or overlap requires compatibility reconciliation. Correction publication proves the leased remote HEAD is an ancestor of the reviewed local HEAD before its one push, performs no PR-create mutation, and directly refetches the existing PR at the successor HEAD. The predelegation does not authorize rebase, amend, scope expansion, another PR, Ready, Merge, retry, or Issue closure. A cursor activates at most one operation but is never its authority.
