...
- The canonical pre-PR Implementer ingress keeps the existing closed 21-field `pre_pr_implementation_authority_v1` as a clean-start authority: its initial named worktree must have zero changed and staged paths. Worktree inspection preserves every changed path with an internal `PRESENT` or `ABSENT` state and acquires a filtered Git blob only for present paths, so legacy deleted or rename-away paths reach the existing binding guard without a missing-file hash failure. An already-prepared delta is admissible only through the closed 23-field form that adds both `initial_worktree_mode: EXACT_AUTHORIZED_DELTA` and `initial_delta_manifest_sha256`. V1 is present-file-only: any exact-delta path with `ABSENT` state stops as `pre_pr_authorized_delta_non_present_path_unsupported`; it has no deletion marker, rename representation, tombstone, or alternate manifest. The unchanged manifest digest is SHA-256 over the sorted present-file entries encoded as repository path, NUL, filtered Git blob OID, and LF. Initial and later rebinds for this form require exact base and branch, changed paths equal to the full authorized list, zero staged and untracked paths, and the same manifest digest. Missing identity fields, arbitrary dirty bytes, a subset, extra or missing paths, wrong bytes, or any other identity drift remains `role_dispatch_binding_changed`; legacy Worker and later authorized-subset rebind behavior is unchanged.
