...
The closed Review publication route fetches the authenticated actor and PR author and enumerates both canonical surfaces for the exact Task, PR, and HEAD before considering mutation. Exactly one byte-identical authority is reused with zero mutation. If none exists, the semantic Review result is not publication authority: the route requires a direct Product Owner `task_assignment` granting `REVIEW_AUTHORITY_PUBLICATION` for the exact repository, Task, PR, HEAD, branch, base, scope, actor, one surface, closed `APPROVE / 0 / 0 / 0` body, and `operation_count = 1`. The runner repeats the live identity and assignment checks immediately before mutation. A distinct actor may use only the assignment-authorized PR `APPROVE` endpoint; a self actor may use only the assignment-authorized `TASK_ISSUE_COMMENT` surface because GitHub prohibits PR self-approval. Missing authority, duplicates, conflicts, malformed records, actor or live-binding drift fail closed before mutation. A created resource is refetched and required to preserve body, parser, resource, actor, and exact-HEAD identity. Publication failure never switches surfaces or retries.
