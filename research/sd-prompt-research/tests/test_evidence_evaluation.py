from __future__ import annotations

import copy
import sys
import unittest
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "scripts"))

import evidence_evaluation as evaluation  # noqa: E402


def condition(region: str, *states: str) -> dict:
    return {"region": region, "allowed_states": list(states)}


def rule(branch: str, *conditions: dict) -> dict:
    return {
        "rule_id": "evidence_rule.fixture.contact",
        "visibility_prerequisite": {branch: list(conditions)},
    }


def panel(panel_id: int = 1, **regions: str) -> dict:
    return {
        "panel_id": panel_id,
        "visibility_status": "available",
        "visible_regions": regions,
    }


def metadata(*panels: dict, run_id: str = "BRG-TEST-A", available: bool = True) -> dict:
    return {
        "run_id": run_id,
        "visibility_status": "available" if available else "unavailable",
        "panels": list(panels),
    }


class EvidenceEvaluationTests(unittest.TestCase):
    def evaluate(self, value: dict, artifact: dict | None) -> evaluation.VisibilityEvaluation:
        return evaluation.evaluate_visibility_prerequisite(
            value,
            artifact,
            run_id="BRG-TEST-A",
            panel_id=1,
        )

    def test_condition_explicit_states_are_evaluable(self) -> None:
        for state, expected in (
            ("visible", evaluation.EvaluationStatus.SATISFIED),
            ("partial", evaluation.EvaluationStatus.SATISFIED),
            ("unclear", evaluation.EvaluationStatus.UNSATISFIED),
            ("not_visible", evaluation.EvaluationStatus.UNSATISFIED),
        ):
            with self.subTest(state=state):
                result = self.evaluate(
                    rule("all_of", condition("hands", "visible", "partial")),
                    metadata(panel(hands=state)),
                )
                self.assertEqual(expected, result.status)
                self.assertEqual(state, result.conditions[0].actual_state)

    def test_explicit_not_visible_can_satisfy_a_declared_rule(self) -> None:
        result = self.evaluate(
            rule("all_of", condition("hands", "not_visible")),
            metadata(panel(hands="not_visible")),
        )
        self.assertEqual(evaluation.EvaluationStatus.SATISFIED, result.status)

    def test_missing_region_is_not_evaluated_not_not_visible(self) -> None:
        result = self.evaluate(
            rule("all_of", condition("hands", "not_visible")),
            metadata(panel(feet="partial")),
        )
        self.assertEqual(evaluation.EvaluationStatus.NOT_EVALUATED, result.status)
        self.assertIsNone(result.conditions[0].actual_state)

    def test_unsupported_metadata_state_is_not_evaluated(self) -> None:
        result = self.evaluate(
            rule("all_of", condition("hands", "visible")),
            metadata(panel(hands="mostly_visible")),
        )
        self.assertEqual(evaluation.EvaluationStatus.NOT_EVALUATED, result.status)
        self.assertIsNone(result.conditions[0].actual_state)

    def test_all_of_truth_table(self) -> None:
        cases = (
            (
                metadata(panel(hands="visible", feet="visible")),
                evaluation.EvaluationStatus.SATISFIED,
            ),
            (
                metadata(panel(hands="partial", feet="visible")),
                evaluation.EvaluationStatus.UNSATISFIED,
            ),
            (
                metadata(panel(hands="visible")),
                evaluation.EvaluationStatus.NOT_EVALUATED,
            ),
        )
        value = rule(
            "all_of",
            condition("hands", "visible"),
            condition("feet", "visible"),
        )
        for artifact, expected in cases:
            with self.subTest(expected=expected):
                self.assertEqual(expected, self.evaluate(value, artifact).status)

    def test_any_of_truth_table(self) -> None:
        cases = (
            (
                metadata(panel(hands="visible", feet="partial")),
                evaluation.EvaluationStatus.SATISFIED,
            ),
            (
                metadata(panel(hands="partial", feet="partial")),
                evaluation.EvaluationStatus.UNSATISFIED,
            ),
            (
                metadata(panel(hands="partial")),
                evaluation.EvaluationStatus.NOT_EVALUATED,
            ),
        )
        value = rule(
            "any_of",
            condition("hands", "visible"),
            condition("feet", "visible"),
        )
        for artifact, expected in cases:
            with self.subTest(expected=expected):
                self.assertEqual(expected, self.evaluate(value, artifact).status)

    def test_all_conditions_are_reported_in_stable_region_order(self) -> None:
        first = rule(
            "any_of",
            condition("hands", "visible"),
            condition("arms", "visible"),
        )
        second = rule(
            "any_of",
            condition("arms", "visible"),
            condition("hands", "visible"),
        )
        artifact = metadata(panel(hands="visible"))
        first_result = self.evaluate(first, artifact)
        second_result = self.evaluate(second, artifact)
        self.assertEqual(first_result, second_result)
        self.assertEqual(["arms", "hands"], [item.region for item in first_result.conditions])
        self.assertEqual(
            [evaluation.EvaluationStatus.NOT_EVALUATED, evaluation.EvaluationStatus.SATISFIED],
            [item.status for item in first_result.conditions],
        )

    def test_duplicate_region_stops_rule_evaluation(self) -> None:
        result = self.evaluate(
            rule(
                "all_of",
                condition("hands", "visible"),
                condition("hands", "partial"),
            ),
            metadata(panel(hands="visible")),
        )
        self.assertEqual(evaluation.EvaluationStatus.NOT_EVALUATED, result.status)
        self.assertEqual((), result.conditions)
        self.assertEqual(evaluation.EVIDENCE_RULE_INVALID, result.diagnostics[0].code)

    def test_invalid_or_empty_branch_stops_rule_evaluation(self) -> None:
        for prerequisite in ({}, {"all_of": []}, {"all_of": [], "any_of": []}):
            with self.subTest(prerequisite=prerequisite):
                result = self.evaluate({"visibility_prerequisite": prerequisite}, metadata(panel()))
                self.assertEqual(evaluation.EVIDENCE_RULE_INVALID, result.diagnostics[0].code)

        result = self.evaluate(
            rule("all_of", condition("hands", "mostly_visible")),
            metadata(panel(hands="visible")),
        )
        self.assertEqual(evaluation.EVIDENCE_RULE_INVALID, result.diagnostics[0].code)

    def test_exact_run_and_unique_panel_binding(self) -> None:
        value = rule("all_of", condition("hands", "visible"))
        self.assertEqual(
            evaluation.EvaluationStatus.SATISFIED,
            self.evaluate(value, metadata(panel(hands="visible"))).status,
        )
        for artifact, reason in (
            (metadata(panel(hands="visible"), run_id="OTHER"), "run_binding_failed"),
            (metadata(panel(2, hands="visible")), "panel_binding_failed"),
            (
                metadata(panel(hands="visible"), panel(hands="visible")),
                "panel_binding_ambiguous",
            ),
        ):
            with self.subTest(reason=reason):
                result = self.evaluate(value, artifact)
                self.assertEqual(evaluation.EvaluationStatus.NOT_EVALUATED, result.status)
                self.assertEqual(reason, result.not_evaluated_reason)

    def test_unavailable_root_or_panel_is_not_evaluated(self) -> None:
        value = rule("all_of", condition("hands", "visible"))
        unavailable_panel = panel(hands="visible")
        unavailable_panel["visibility_status"] = "unavailable"
        cases = (
            (None, "visibility_metadata_missing"),
            (metadata(panel(hands="visible"), available=False), "visibility_root_unavailable"),
            (metadata(unavailable_panel), "visibility_panel_unavailable"),
        )
        for artifact, reason in cases:
            with self.subTest(reason=reason):
                result = self.evaluate(value, artifact)
                self.assertEqual(evaluation.EvaluationStatus.NOT_EVALUATED, result.status)
                self.assertEqual(reason, result.not_evaluated_reason)

    def test_evaluation_does_not_mutate_inputs(self) -> None:
        value = rule("all_of", condition("hands", "visible"))
        artifact = metadata(panel(hands="visible"))
        before_rule = copy.deepcopy(value)
        before_artifact = copy.deepcopy(artifact)
        self.evaluate(value, artifact)
        self.assertEqual(before_rule, value)
        self.assertEqual(before_artifact, artifact)

    def overclaim(
        self,
        visibility: evaluation.VisibilityEvaluation,
        *,
        observed_value: str | None = "supporting",
        policy: evaluation.PolicyEvaluationStatus = evaluation.PolicyEvaluationStatus.NO_VIOLATION,
        prerequisite_values: tuple[str, ...] = ("supporting",),
        fallback_values: tuple[str, ...] = ("unclear",),
        allowed_values: tuple[str, ...] | None = ("supporting", "unclear"),
        bound: bool = True,
    ) -> evaluation.OverclaimEvaluation:
        return evaluation.evaluate_overclaim(
            visibility,
            observed_value=observed_value,
            rubric_allowed_values=allowed_values,
            prerequisite_values=prerequisite_values,
            fallback_values=fallback_values,
            rubric_binding_succeeded=bound,
            policy_status=policy,
        )

    @staticmethod
    def visibility(status: evaluation.EvaluationStatus) -> evaluation.VisibilityEvaluation:
        return evaluation.VisibilityEvaluation(status=status, conditions=())

    def test_satisfied_visibility_with_clear_policy_is_no_violation(self) -> None:
        result = self.overclaim(self.visibility(evaluation.EvaluationStatus.SATISFIED))
        self.assertEqual(evaluation.OverclaimStatus.NO_VIOLATION, result.status)
        self.assertEqual((), result.diagnostics)

    def test_unsatisfied_visibility_with_allowed_fallback_is_no_violation(self) -> None:
        result = self.overclaim(
            self.visibility(evaluation.EvaluationStatus.UNSATISFIED),
            observed_value="unclear",
        )
        self.assertEqual(evaluation.OverclaimStatus.NO_VIOLATION, result.status)

    def test_confirmed_assertive_overclaim_has_error_then_warning(self) -> None:
        result = self.overclaim(
            self.visibility(evaluation.EvaluationStatus.UNSATISFIED),
            policy=evaluation.PolicyEvaluationStatus.VIOLATION,
        )
        self.assertEqual(evaluation.OverclaimStatus.VIOLATION, result.status)
        self.assertEqual(
            [evaluation.EVIDENCE_OBSERVATION_OVERCLAIM, evaluation.EVIDENCE_VISIBILITY_INSUFFICIENT],
            [item.code for item in result.diagnostics],
        )
        self.assertEqual(
            [evaluation.Severity.ERROR, evaluation.Severity.WARNING],
            [item.severity for item in result.diagnostics],
        )

    def test_missing_or_invalid_observation_value_is_not_evaluated(self) -> None:
        visibility = self.visibility(evaluation.EvaluationStatus.SATISFIED)
        missing = self.overclaim(visibility, observed_value=None)
        self.assertEqual(evaluation.OverclaimStatus.NOT_EVALUATED, missing.status)
        self.assertEqual((), missing.diagnostics)

        invalid = self.overclaim(visibility, observed_value="invented")
        self.assertEqual(evaluation.OverclaimStatus.NOT_EVALUATED, invalid.status)
        self.assertEqual(evaluation.EVIDENCE_RUBRIC_MAPPING_INVALID, invalid.diagnostics[0].code)

    def test_rubric_or_policy_unavailable_is_not_evaluated(self) -> None:
        visibility = self.visibility(evaluation.EvaluationStatus.SATISFIED)
        for result in (
            self.overclaim(visibility, bound=False),
            self.overclaim(visibility, allowed_values=None),
            self.overclaim(
                visibility,
                policy=evaluation.PolicyEvaluationStatus.NOT_EVALUATED,
            ),
        ):
            with self.subTest(reason=result.not_evaluated_reason):
                self.assertEqual(evaluation.OverclaimStatus.NOT_EVALUATED, result.status)
                self.assertEqual(
                    evaluation.EVIDENCE_RUBRIC_MAPPING_INVALID,
                    result.diagnostics[0].code,
                )

    def test_visibility_not_evaluated_propagates_only_to_overclaim(self) -> None:
        visibility = self.visibility(evaluation.EvaluationStatus.NOT_EVALUATED)
        result = self.overclaim(visibility)
        self.assertEqual(evaluation.OverclaimStatus.NOT_EVALUATED, result.status)
        self.assertEqual("visibility_not_evaluated", result.not_evaluated_reason)
        self.assertEqual(evaluation.EvaluationStatus.NOT_EVALUATED, visibility.status)

        result = self.overclaim(visibility, bound=False)
        self.assertEqual("visibility_not_evaluated", result.not_evaluated_reason)
        self.assertEqual((), result.diagnostics)

    def test_cross_array_overlap_is_rule_invalid(self) -> None:
        result = self.overclaim(
            self.visibility(evaluation.EvaluationStatus.UNSATISFIED),
            prerequisite_values=("supporting", "unclear"),
            fallback_values=("unclear",),
        )
        self.assertEqual(evaluation.OverclaimStatus.NOT_EVALUATED, result.status)
        self.assertEqual(evaluation.EVIDENCE_RULE_INVALID, result.diagnostics[0].code)

    def test_undefined_rule_policy_combination_is_not_guessed(self) -> None:
        result = self.overclaim(
            self.visibility(evaluation.EvaluationStatus.UNSATISFIED),
            policy=evaluation.PolicyEvaluationStatus.NO_VIOLATION,
        )
        self.assertEqual(evaluation.OverclaimStatus.NOT_EVALUATED, result.status)
        self.assertEqual(evaluation.EVIDENCE_RUBRIC_MAPPING_INVALID, result.diagnostics[0].code)


if __name__ == "__main__":
    unittest.main()
