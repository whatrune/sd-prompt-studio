...
- The canonical pre-PR Implementer ingress keeps the existing closed 21-field `pre_pr_implementation_authority_v1` as a clean-start authority: its initial named worktree must have zero changed and staged paths. An already-prepared delta is admissible only through the closed 23-field form that adds both `initial_worktree_mode: EXACT_AUTHORIZED_DELTA` and `initial_delta_manifest_sha256`. That digest is SHA-256 over the sorted authorized entries encoded as repository path, NUL, filtered Git blob OID, and LF. Initial and later rebinds for this form require exact base and branch, changed paths equal to the full authorized list, zero staged and untracked paths, and the same manifest digest. Missing identity fields, arbitrary dirty bytes, a subset, extra or missing paths, wrong bytes, or any identity drift remains `role_dispatch_binding_changed`; legacy Worker and later authorized-subset rebind behavior is unchanged.
