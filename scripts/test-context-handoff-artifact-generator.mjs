...
try {
 ...
 const incomplete={...checkpointInput,material_facts:[{...unresolved,evidence_state:'unverified',provenance_ids:[]}]}; incomplete.checkpoint_record_builder_input_ref=await artifacts.generateCheckpointRecordBuilderInputRef(incomplete)
 ...
 assert.ok(bundle.generated_components.every(artifacts.validateGeneratedComponentV1),JSON.stringify(bundle.generated_components.find(component=>component.artifact.component_kind==='canonical_record_manifest'))); assert.ok(artifacts.validateContextHandoffBundleBuilderResultV1(bundle))
