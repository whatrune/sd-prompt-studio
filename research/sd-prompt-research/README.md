...
Research Explorer向けのLocal Companion Serviceは、Research Artifactをread-onlyで探索し、Derived Indexを生成して、既存Frontend buildと`/api/research/*`を同一Originで配信します。公開Previewへ実Research Dataを含めず、ブラウザへfilesystem pathを渡しません。
