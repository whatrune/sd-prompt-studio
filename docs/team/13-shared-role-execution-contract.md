...
Normal Task execution uses the Product Owner's Task-start authorization for implementation/Research, validation, commit, push, one non-Draft PR, and same-Task correction within the exact cumulative scope. No prepublication Independent Review, terminal machine handoff, external event file, or thread/turn/cursor identity is required as authority. The existing normal execution owner checks the canonical Task, actor, assigned branch/registered worktree, HEAD, fresh base, exact paths, and PASS validation before commit or publication. Publication preserves the validated commit; corrections use the existing PR and an exact remote-head lease. A new HEAD requires current checks and Fresh Review. Scope expansion, semantic conflict, failed validation, stale identity, and ambiguous mutation stop the lane.

Fresh Review remains mandatory after current-HEAD checks PASS. Its closed exact Task/PR/HEAD APPROVE with blocking / remaining / unknown = 0 / 0 / 0 activates direct canonical Review publication under the Task's stable predelegation. The existing gh api transport publishes at most one Review, directly refetches its body/resource/actor, and re-enumerates Review authorities; exactly one identical current authority is reused without mutation, and duplicate or conflicting Reviews stop before preflight. V2 assignment comments and logical-equivalence bookkeeping are not required. Exact base/branch/scope, required checks, zero active unresolved non-outdated threads, mergeability, actor and permitted surface are checked before publication and preflight. No alternate-surface fallback or ambiguous retry is permitted.

The Product Owner's direct MERGE Task #X / PR #Y instruction is the semantic Merge Decision and authorizes publication of its canonical record without a second approval. The existing serializer binds the freshly verified HEAD, base, scope and Review identity; publishCanonicalMergeDecisionV1 publishes or reuses that exact record with one direct GitHub comment route. Only the existing protected GitHub workflow performs Merge. Preflight remains read-only and MERGE_READY stops for the Product Owner. Post-Merge verification, local-main sync, worktree cleanup and predelegated Task closure retain their existing owners and policies.
...
Every command session a Role launches with its working directory at or beneath the exact Task worktree remains execution-local ownership of that Role. Before returning any successful terminal Role result, including `completed` or `APPROVE`, the same owning execution MUST require every such session to be terminal with its exact spawned process tree absent. A returned session identifier, pending command, interrupted wrapper, or detached descendant is nonterminal and prohibits successful handoff. Ordinary Task-worktree commands MUST NOT detach descendants; a command that must remain active MUST use an existing bounded execution owner that can terminate and prove absence of its exact process tree. If terminality or absence cannot be proved, the Role MUST use the existing applicable `blocked` or `failed` Result Handoff, and ownership remains with the same Role execution rather than transferring to Integrated Lead or post-Merge cleanup. This rule adds no OS-wide process discovery, broad process killing, retry, fallback, daemon, scheduler, or cleanup-side ownership detection and does not change worktree-cleanup semantics.
...
Before that CREATE freezes scope, its request MUST include one closed bounded artifact-dependency consideration. The request explicitly states whether the Task changes an existing canonical artifact contract and identifies that artifact's existing semantic owner. It MUST explicitly account for each same-owner consumer category: serializer / writer, validator / schema, renderer / exporter, registrar / indexer, and current-state / production consumer. Each category is either exact `NOT_APPLICABLE` or a non-empty list of repository-relative paths already present in `authorized_paths`; a listed path outside that scope fails admission and MUST NOT expand scope. A no-contract-change acknowledgement requires `NOT_APPLICABLE` for the owner and every category. A contract-change acknowledgement requires a named owner and at least one applicable category. The serializer preserves the complete consideration in canonical Task Markdown and as a versioned structured binding in the existing normal-execution predelegation, whose required versioned `cumulative_scope` discriminator is outside that optional grant field. For that guarded shape, the parser MUST require the discriminator, structured consideration, authorized paths, and exact generated Markdown suffix to agree. Bound Canonical Task number `715` is the first available guarded identity in this repository; that immutable server-issued boundary requires the guarded shape for Task `715` and later, so coordinated removal or downgrade of every body-local indicator still fails closed. This is a bounded formation acknowledgement, not repository discovery: it adds no scanner, graph traversal, inferred dependency, lifecycle owner, fallback authority, or automatic scope amendment. Historical Canonical Task bodies before that boundary with the legacy discriminator and without the structured binding remain admissible under their existing shape, but a new CREATE without the acknowledgement fails before mutation.
