...
Normal-execution authority is the stable Product Owner `task_assignment` predelegation in the Canonical Task body, not the `wait_threads` cursor. It binds the exact repository, Task, initial fresh base, branch, registered worktree path, cumulative scope, actor, and `BOUNDED_EXECUTION_IDENTITY_V1`. Each commit or publication activation must refetch the exact consumed target/cursor and admit its Result Handoff or Independent Review; request JSON cannot supply either decision. It permits one exact validated-tree commit per newly admitted execution identity, one unchanged initial push plus non-Draft PR creation after prepublication approval, and one exact leased successor push to that existing PR per reviewed same-task correction identity. The initial base remains the authority anchor; a freshly observed descendant `origin/main` can replace the execution base only when the intervening path set is disjoint from the Task scope, and the execution identity separately binds the local rebound HEAD and existing PR's leased remote HEAD. Divergence or overlap stops for compatibility reconciliation. The predelegation does not permit scope expansion, rebase, amend, another PR, Ready mutation, Merge, retry, or Issue closure. Architecture or scope change and semantic base conflict remain Product Owner or Architect boundaries.
