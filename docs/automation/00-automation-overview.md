...
| Stable Platform Bootstrap Kernel V1 (`MARK_READY`) | Protected Transition Admission admits one exact Task/PR/HEAD/base/four-path Product Owner authority only after a canonical `SELF_HOSTING_UNREACHABLE` proof, claims it once, and delegates the sole mutation to the existing Ready operator | bounded `workflow_dispatch.platform_bootstrap_mark_ready_resume` route | repository-reachable after the implementing exact HEAD reaches the default branch |
...
- `platform_bootstrap_mark_ready_resume` is Stable Platform Bootstrap Kernel V1's closed admission for a self-amending PTA PR that cannot use its normal current-generation Ready route because the required capability is not yet on the default branch. A self-bound Backend Architect `self_hosting_unreachable_v1` proof binds the exact Task, Draft PR, HEAD, current main, closed four PTA paths, failed normal route/run and first STOP. The owner does not trust those run fields as assertions: on each snapshot it refetches the referenced default-branch `workflow_dispatch` run, authenticates its repository, workflow, main SHA, failed admission job, exact five-job manifest, and terminal log binding to the same Task, PR, HEAD, zero-mutation STOP, and recorded reason. A later self-bound Product Owner `stable_platform_bootstrap_authority_v1` binds that proof by URL and SHA-256, the same identities, `kernel: STABLE_PLATFORM_BOOTSTRAP_KERNEL_V1`, `operation: MARK_READY`, and a fresh canonical Independent Review with `APPROVE / 0 / 0 / 0`.
- Kernel V1's operation catalog contains `MARK_READY` only. Unknown operations, missing/duplicate/drifted proofs, proof or authority digest mismatch, any identity/scope drift, or any claimed lawful normal continuation fail closed. After successful admission, the owner freshly requires successful applicable checks, zero active non-outdated threads, and valid mergeability before it publishes one self-bound `stable_platform_bootstrap_consumption_v1` claim.
- The consumption claim makes the authority single-use before the protected mutation. Consumption is intentionally bound to the attempt: a rejected or interrupted attempt remains consumed and has no automatic retry or successor issuance. After a second complete rebind of authority, Review, main, PR, paths, checks, and threads, the route passes a module-authenticated action to the existing `executeReadyTransitionOperatorV1`. That operator remains the only owner of `markPullRequestReadyForReview`, the before/after Draft-state guards, and exact-HEAD refetch. The workflow still relies on GitHub's real `pull_request.ready_for_review` event; no event is synthesized.
- This permanent Kernel admission does not read, write, synthesize, or repair Task-state or validation-generation evidence. It does not authorize Merge and does not change `MINIMAL_GOVERNANCE_V1`. Ordinary product PRs and arbitrary stale-state PRs are outside its proof-bound closed four-path boundary. The one-time external Seed used to place the first Kernel implementation on the default branch is not represented by a workflow transition, authority parser, or reusable repository capability and is permanently closed after that merge.
