...
顔周辺の表現は、見た目の責務に応じて次のslotへ分離します。

- `face_color`: 顔全体の色変化。`pale face`や`full-face blush`など
- `blush_type`: 赤面・化粧・線表現。`rosy cheeks`、`nose blush`、`blush lines`、`blush stickers`など
- `face_state`: 一時状態・漫画的記号。`tears`、`sweatdrop`、`vein pop`など
- `cheek_state`: 頬の物理的な形や動作。`puffed cheeks`、`sucked-in cheeks`、`biting cheek`など

`face_color`と`blush_type`は、`full-face blush`のように一つのタグが両方を占有できます。`face_state`は複数の一時表現が成立し得るためmultiple、`cheek_state`は同じ頬形状を重ねないためsingleとします。

## 目・まつ毛

目は形状、状態、視線、ハイライト、効果、まぶた、アイラインを分離します。`eye_effect`はglowingだけでなくwet、reflective、sparkling等を含み、複数併用可能です。視線方向の`gaze_direction`と、身体動作を伴うMotion側の`gaze_action`は別slotを維持します。

まつ毛は`eyelash_length`、`upper_eyelashes`、`lower_eyelashes`、`eyelash_shape`、`eyelash_color`をslots.jsonで明示し、色と装飾を併用できるよう`eyelash_decoration`をmultipleで分離します。
