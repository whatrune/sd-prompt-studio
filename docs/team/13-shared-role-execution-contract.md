...
Before that CREATE freezes scope, its request MUST include one closed bounded artifact-dependency consideration. The request explicitly states whether the Task changes an existing canonical artifact contract and identifies that artifact's existing semantic owner. It MUST explicitly account for each same-owner consumer category: serializer / writer, validator / schema, renderer / exporter, registrar / indexer, and current-state / production consumer. Each category is either exact `NOT_APPLICABLE` or a non-empty list of repository-relative paths already present in `authorized_paths`; a listed path outside that scope fails admission and MUST NOT expand scope. A no-contract-change acknowledgement requires `NOT_APPLICABLE` for the owner and every category. A contract-change acknowledgement requires a named owner and at least one applicable category. The serializer preserves the complete consideration in canonical Task Markdown. This is a bounded formation acknowledgement, not repository discovery: it adds no scanner, graph traversal, inferred dependency, lifecycle owner, fallback authority, or automatic scope amendment. Historical Canonical Task bodies remain admissible under their existing shape, but a new CREATE without the acknowledgement fails before mutation.
