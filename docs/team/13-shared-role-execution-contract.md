...
For Review publication, the terminal cursor is a wake-up signal only and is never protected-action authority. For an already completed post-publication Fresh Reviewer execution bound to the exact Task, PR, and current HEAD with `APPROVE / 0 / 0 / 0`, `threadId`, terminal-turn identity, and `wait_threads` cursor are optional continuation diagnostics; their absence or later unavailability does not invalidate that completed Review or block its canonical publication. This exception does not admit an unfinished Review or caller-authored substitute and does not apply to implementation commit, prepublication Review, or corrected-thread resolution, whose existing consumed-event requirements remain mandatory. Authority is the stable Product Owner predelegation plus freshly admitted GitHub state. The exact assignment logical identity is repository + Task + PR + HEAD + protected action + actor + surface + decision. Physical assignment comments with one logical identity and byte-identical canonical semantic payload are equivalent and collapse to one authority; conflicting payloads fail closed. Exact assignment and Review publication remain separate single-attempt mutations with direct refetch, no PATCH, no ambiguous-result retry, and no alternate-surface fallback. Every protected effect requires fresh Task, PR, exact HEAD/base/branch, cumulative scope, the completed Fresh Review, required checks, zero active non-outdated threads, Merge applicability, actor, and surface admission.
