...
| B. Index File生成 | Generatorが必要 | UI読込と検索が高速 | Source Freshness Fingerprintと生成時刻で鮮度を確認できる | AgentとUIが同じRead Modelを利用可能 | Indexのstale検知と再生成導線が必要 |
...
- 存在する場合に限り、既存Pipelineが生成したResearch/Audit Hashへの参照
- Relationship先ID
...
### 6.5 Hash Responsibility Boundary

Hash用途をResearch/Audit HashとSource Freshness Fingerprintへ分離し、IndexとUIで混同しない。

#### 6.5.1 Research/Audit Hash

Research/Audit Hashは既存Pipelineで定義済みのHashだけを使用する。用途はResearch監査、Semantic確認、Artifact Binding確認である。Research Explorerの表示対象例は次のとおり。
...
Research/Audit Hashが存在しないArtifactに新しいResearch Hashを追加してはならない。

#### 6.5.2 Source Freshness Fingerprint

Source Freshness FingerprintはDerived Indexの鮮度確認とSource Artifact変更検知だけに使用する。Index GeneratorはIndex生成時のFingerprintを保存し、Artifact読込時またはIndex再検証時に現在値と比較する。

禁止事項：

- Research/Audit Hashとして表示する
- Semantic Equality判定に使用する
- 既存Pipeline Hashの代替として使用する
- Claim、Evidence、Promotion、研究判断へ使用する

Source Freshness FingerprintのAlgorithm、入力Projection、Versioning、比較規則はPR74で定義する。これはRead Modelの鮮度管理概念であり、PR69/PR71のResearch/Audit Hash Contractを変更しない。
...
- Validation Context
- Evaluated At
- 適用可能な既存Research/Audit Hash
- Index Snapshot IDとSource Freshness Fingerprint
- Current Artifactと一致するか

Source Freshness Fingerprintが一致しない過去結果は削除せず`stale`補助Badgeを付ける。`stale`はValidationの合否やResearch/Audit Hashの意味を変更するStatusではなく、現在Artifactへの適用可否を示すUI Metadataである。
...
Query Cacheで管理し、UIから変更しない。Cache keyはArtifact Type、完全ID、Index Snapshot ID、Source Freshness Fingerprintを含む。
...
同じIndex SnapshotとSource Freshness Fingerprintから決定的に再構成できる。独立した正本として永続化しない。
...
| `ArtifactViewer` | Artifact content、format、source freshness state | tab/copy/scroll | Save、format結果の書戻し |
...
- Source Freshness FingerprintがIndex Snapshotから変化した
...
- wildcard CORSを許可しない
- Host allowlistとOrigin allowlistを強制する
- session tokenによってResearch API accessを制限する
- session tokenはHttpOnly Cookie等のBrowser scriptから直接読めない方式で管理する
- session tokenをURL、query parameter、localStorageへ保存しない
...
- Research APIをread-onlyとし、ArtifactまたはPipeline Stateを変更するMutation endpointを提供しない
- Research Explorer Frontendと`/api/research/*`を同一Originで配信する

Derived Index：

- `index_snapshot_id`を持つ
- 各ArtifactのSource Freshness Fingerprintを保持する
- Research/Audit Hashが存在するArtifactだけ、既存Pipeline Hashへの参照を別Fieldで保持する
- Source Freshness FingerprintとResearch/Audit Hashを同じField、Label、判定処理へ混在させない
- Index生成後のArtifact変更を`stale`として検出する
- Index SnapshotまたはSource Freshness Fingerprint mismatch時に処理を停止し、古いStatusや本文を正常値として表示しない
...
- Source Freshness FingerprintのAlgorithm、入力Projection、Versioning、比較規則
