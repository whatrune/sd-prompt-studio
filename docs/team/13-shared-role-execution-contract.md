...
A Product Owner may place exactly one stable materialization-only `REVIEW_AUTHORITY_PUBLICATION` predelegation in the Canonical Task body. It is inactive until a new Fresh exact-HEAD `APPROVE / 0 / 0 / 0` `wait_threads` terminal cursor is consumed. The terminal must bind the exact dispatched Review target thread and host together with the repository, Task, PR, HEAD, base, branch, scope, and Review result; a cursor carrying another execution identity is `NO_ADVANCE`. After activation, the consumer freshly derives the exact PR, HEAD, base, branch, scope, actor, permitted surface, and Review. It may reuse one uniquely valid exact assignment or create exactly one top-level Task Issue comment under the existing `task_assignment` contract. The complete pre-create body uses `canonical_record: GITHUB_RESOURCE`; only the successful CREATE response and byte-identical direct refetch bind the admitted comment ID and URL. No Task-body materialization PATCH, assignment-comment PATCH, alternate-surface fallback, or ambiguous-write retry is permitted. Legacy exact Task-body assignments remain readable, but a stale assignment cannot authorize a changed HEAD or base. Applicable duplicates, conflicts, malformed records, cursor replay, or live drift stop before Review publication.
