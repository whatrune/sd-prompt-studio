...
Canonical Task Issue creation uses one Node-owned composite serializer and one narrowly bounded server-assigned-number self-binding PATCH. The CREATE body is a structurally validated but non-authoritative `UNBOUND_CREATE` representation with reserved Task number `0`. After CREATE and direct refetch equality, `BOUND_FINAL` derives the Task Authority number and every predelegation Task-number field from the single returned Issue number. Only Task Authority `task_issue` and predelegation `task_id`, `authority_source`, `canonical_record`, and `allowed_changes.task_issue` may change. The complete Markdown, exact sorted scope, all other authority semantics, fences, encoding, and newline policy MUST remain stable. CREATE and PATCH are each attempted at most once, use the same UTF-8 file-backed serializer output, and require direct refetch equality; ambiguity stops without retry or recovery reconstruction. The final Issue is canonical only after exactly one Task Authority and exactly one predelegation parse and self-bind to the assigned number. This exception does not permit PATCHing Review, Merge Decision, derived exact-assignment, or other protected-action records.
